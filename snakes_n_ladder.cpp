...
int main() {
	...
	w(test) {
		int N, M;
		sfi(N);
		pair<int, int> *ladders = new pair<int, int> [N];
		f(i, N)
			scanf("%d%d", &ladders[i].first, &ladders[i].second);
		sfi(M);
		pair<int, int> *snakes = new pair<int, int> [M];	
		f(i, M)
			scanf("%d%d", &snakes[i].first, &snakes[i].second);
	}
	return 0;
}